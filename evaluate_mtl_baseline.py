...
# Import the MTLWrapper class.
from lib.mtl.trainer import MTLWrapper

# --- Fix for unpickling error: Can't get attribute 'BaseWrapper' ---
try:
    from lib.mtl.trainer import BaseWrapper
except ImportError:
    BaseWrapper = MTLWrapper
# --------------------------------------------------------------
...
class MTLEvaluator:
    def __init__(self, mtl_wrapper, baseline_wrapper, config):
        ...
        Parameters:
          ...
          config (dict): Evaluation configuration for additional parameters.
        """
    ...
    def compute_overall_metrics(self, wrapper):
        ...
        for subj, res in wrapper.results_by_subject.items():
            ...
            if not (isinstance(res, dict) and "ground_truth" in res and "predictions" in res):
                print(f"Warning: Results for subject {subj} not in expected format; skipping.")
                continue
    ...
    def compute_subject_metrics(self):
        ...
        Each record includes: subject, cluster, baseline_accuracy, mtl_accuracy, and difference.
        If no subject-level baseline result is found for a subject, it falls back to pooled (if available)
        ...
        # Check if baseline results are pooled.
        use_pooled = ("pooled" in baseline_dict and len(baseline_dict) == 1)
        for subj, res in self.mtl_wrapper.results_by_subject.items():
            ...
            # Try to get baseline result for current subject.
            if use_pooled:
                baseline_res = baseline_dict["pooled"]
            else:
                baseline_res = baseline_dict.get(subj, None)
                if baseline_res is None and "pooled" in baseline_dict:
                    print(f"Warning: No subject-level result for {subj}, falling back to pooled baseline.")
                    baseline_res = baseline_dict["pooled"]
...
if __name__ == "__main__":
    # Load the saved MTL results.
    with open("mtl_wrapper_results.pkl", "rb") as f:
        mtl_wrapper = pickle.load(f)
    print("Loaded MTL results from mtl_wrapper_results.pkl")

    # Load the baseline results (change the file path for pooled vs. subject-based).
    with open("./trained_models/single_baseline_results.pkl", "rb") as f:
        baseline_results = pickle.load(f)
    print("Loaded pooled baseline results from subject_baseline_results.pkl")
    
    # Uncomment the following lines to use subject-based baseline results instead:
    # with open("./trained_models/subject_baseline_results.pkl", "rb") as f:
    #     baseline_results = pickle.load(f)
    # print("Loaded subject-based baseline results from subject_baseline_results.pkl")
    
    # Load an evaluation configuration if available, or use default.
    try:
        config = OmegaConf.load("config/experiment/mtl.yaml")
        eval_config = config.experiment.evaluators
    except Exception as e:
        print("Failed to load config/experiment/mtl.yaml, using default configuration.")
        eval_config = {}
